...
### Features
- **마일스톤 여러 개를 한 번에 캘린더에 추가** (카드 목록, `frontend/js/ddays.js`)
  - 마일스톤 목록 각 행에 체크박스 추가 → "📅 선택한 마일스톤 캘린더에 추가" 버튼
  - 1개 선택 → Google 캘린더 등록 페이지 바로 열기 (기존 방식)
  - 여러 개 선택 → 종일 일정들을 담은 **.ics 파일 1개로 내려받기** → 캘린더 가져오기로 일괄 등록
  - 마일스톤 카드의 개별 📅 / 메인 📅 버튼은 체크박스 방식으로 통합

### UX 개선 (마일스톤 선택 "안 됨" 신고 대응)
- 추가/수정 폼의 마일스톤 프리셋을 **작은 체크박스 → 큰 칩(chip) 형태**로 변경
  - 선택 시 칩 전체가 파란 테두리+배경으로 강조 → 모바일에서 선택 상태가 명확히 보임
  - 원인 추적 결과 코드/저장 로직은 정상이었고, 작은 체크박스의 선택 상태가 잘 안 보이던 것 + 캐시 문제로 판단
- `CACHE_NAME` v8 → v9

### 비고
- 프론트엔드 전용 변경 (API/DB/백엔드 변경 없음 → TRD 미변경)
